...
public class MinistryServiceImpl implements MinistryService {
        ...
        public Ministry createMinistry(CreateMinistryRequest req) {
            ministryRepository.findByName(req.getName()).ifPresent(m -> { throw new IllegalArgumentException("Ministry name exists"); });
        ...
        private void persistAssignments(UUID ministryId, List<MemberAssignmentDto> assignments) {
            ...
            Set<UUID> incoming = assignments.stream().map(MemberAssignmentDto::getMemberId).collect(Collectors.toSet());
            List<MinistryMember> existing = ministryMemberRepository.findAllByMinistryId(ministryId);

            Map<UUID, MinistryMember> existingMap = existing.stream().collect(Collectors.toMap(MinistryMember::getMemberId, mm -> mm));

            for (MemberAssignmentDto a : assignments) {
                UUID memberId = a.getMemberId();
    ...
    private List<PotentialMemberDto> potentialMembersFallback(Throwable e) {
        ...
        @Override
        @CircuitBreaker(name = "memberServiceClient", fallbackMethod = "potentialMembersFallback")
        public List<PotentialMemberDto> getPotentialMembers(UUID ministryId) {
            Ministry m = ministryRepository.findById(ministryId).orElseThrow();
            Map<String,String> params = new HashMap<>();
            var c = m.getCriteria();
            if (c != null) {
        ...
        private List<PotentialMemberDto> potentialMembersFallback(UUID ministryId, Throwable t) {
            return Collections.emptyList();
        }

        // highest role logic similar to earlier...
        @Override
        public Optional<Map<String,Object>> highestRoleForMember(UUID memberId) {
